...
class PostureMonitor:
    ...
    def incenter(self, a, b, c):
        side_a = self.euclidean(b, c)
        side_b = self.euclidean(a, c)
        side_c = self.euclidean(a, b)
        px = (side_a * a[0] + side_b * b[0] + side_c * c[0]) / (side_a + side_b + side_c)
        py = (side_a * a[1] + side_b * b[1] + side_c * c[1]) / (side_a + side_b + side_c)
        return np.array([px, py])

    def analyze_posture(self, kp, frame):
        left_ear, right_ear = kp[LEFT_EAR], kp[RIGHT_EAR]
        left_shoulder, right_shoulder = kp[LEFT_SHOULDER], kp[RIGHT_SHOULDER]

        left_face_mid = self.incenter(kp[LEFT_EYE], kp[LEFT_EAR], kp[NOSE])
        right_face_mid = self.incenter(kp[RIGHT_EYE], kp[RIGHT_EAR], kp[NOSE])

        s1 = self.euclidean(left_ear, left_shoulder)
        s2 = self.euclidean(right_ear, right_shoulder)
        s3 = self.euclidean(left_shoulder, right_shoulder)
        f2 = (s1 + s2) / s3 if s3 > 0 else 0
    ...
    def start(self):
        ...
        while True:
            ...
            # check posture only every self.check_interval
            if time.time() - last_check >= self.check_interval:
                ...
                for r in results:
                    ...
                    kp = r.keypoints.xy.cpu().numpy()[0]
                    frame, status = self.analyze_posture(kp, frame)
                    ...
                    if is_bad:
                        ...
                        if recent_bad >= 4 and elapsed >= 30 and not self.notification_sent:
                            print("⚠️ Bad posture detected for 30+ seconds. Remind user.")
                            self.notification_sent = True
                            # snooze example:
